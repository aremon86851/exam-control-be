...
import type { ICreateResult, IUpdateResult, IResultFilters } from "./result.interface"
...
const getAllResults = async (
  ...
  const whereConditions: Prisma.ResultWhereInput = andConditions.length > 0 ? { AND: andConditions } : {}
...
const updateResult = async (id: string, payload: IUpdateResult): Promise<Result> => {
  const result = await prisma.result.findUnique({
    where: { id },
  })
